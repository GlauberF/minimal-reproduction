...
export class VimboNavigationService {
    ...
    register(key, navigation): void {
        ...
        if (this._registry[key]) {
            console.error(
                `[register] - The navigation with the key '${key}' already exists. Either unregister it first or use a unique key.`
            );
    ...
    unregister(key): void {
        ...
        if (!this._registry[key]) {
            console.warn(
                `[unregister] - The navigation with the key '${key}' doesn't exist in the registry.`
            );
        }
    ...
    getNavigation(key): any {
        ...
        if (!this._registry[key]) {
            console.warn(
                `[getNavigation] - The navigation with the key '${key}' doesn't exist in the registry.`
            );
    ...
    getFlatNavigation(
        navigation,
        flatNavigation: _vimboNavigationItem[] = []
    ): any {
    ...
    getCurrentNavigation(): any {
        if (!this._currentNavigationKey) {
            console.warn(
                `[getCurrentNavigation] - The current navigation is not set.`
            );
    ...
    setCurrentNavigation(key): void {
        ...
        if (!this._registry[key]) {
            console.warn(
                `[setCurrentNavigation] - The navigation with the key '${key}' doesn't exist in the registry.`
            );
    ...
    addNavigationItem(item, id): void {
        ...
        // tslint:disable-next-line:prefer-const
        let parent: any = this.getNavigationItem(id);
